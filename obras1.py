...
def main():
        ...
        #st.dataframe(df)
        choice = st.sidebar.radio("Escolha um contrato", options=df.itertuples(), format_func=lambda x : f"{x.Contrato[5:]} - {str(x.Fornecedor)[:20]}")
        ...
        with t2:
           ...
           for post in response1.data:
               ...
               col[0].write("___")
               col[1].markdown(f"{post['datamedicao']}", text_alignment="center")
               col[1].write("___")
